 ...
 _is_handshaking(false)
{
    ...
    switch(_network_stack) {
        ...
        case M2MInterface::LwIP_IPv4:
            tr_debug("M2MConnectionHandlerPimpl::M2MConnectionHandlerPimpl -- SOCKET_STACK_LWIP_IPV4");
        ...
        case M2MInterface::Unknown:
            ...
            break;
        default:
            tr_debug("M2MConnectionHandlerPimpl::M2MConnectionHandlerPimpl -- SOCKET_STACK_UNKNOWN");
            break;
    }
    ...
    if(_binding_mode == M2MInterface::TCP ||
       _binding_mode == M2MInterface::TCP_QUEUE ){
        ...
        tr_debug("M2MConnectionHandlerPimpl::M2MConnectionHandlerPimpl -- SOCKET_STREAM");
    }else{
        _mbed_socket = new MbedSocket(_socket_stack, SOCKET_DGRAM);
        _mbed_socket->open(socket_family, SOCKET_DGRAM);
        tr_debug("M2MConnectionHandlerPimpl::M2MConnectionHandlerPimpl -- SOCKET_DGRAM");
    }
...
{
    tr_debug("M2MConnectionHandlerPimpl::bind_connection");
                                                      ...
                                                      const M2MSecurity* security)
{
    ...
    tr_debug("M2MConnectionHandlerPimpl::resolve_server_address is: %s, port is %d", server_address.c_str(), server_port);
                                     ...
                                     sn_nsdl_addr_s *address)
{
    ...
    if( address == NULL || data == NULL){
        tr_debug("M2MConnectionHandlerPimpl::send_data -- no address/no data to handle");
        return false;
    }
    socket_error_t error = SOCKET_ERROR_NONE;
    if( _use_secure_connection ){
        if( _security_impl->send_message(data, data_len) > 0){
            tr_debug("M2MConnectionHandlerPimpl::send_data::secure -- sending data");
            error = SOCKET_ERROR_NONE;
        }else{
            tr_debug("M2MConnectionHandlerPimpl::send_data::secure -- SOCKET_ERROR_UNKNOWN");
        ...
        if(_binding_mode == M2MInterface::TCP ||
           _binding_mode == M2MInterface::TCP_QUEUE){
            ...
            tr_debug("M2MConnectionHandlerPimpl::send_data::unsecure TCP -- sending data");
            ...
            tr_debug("M2MConnectionHandlerPimpl::send_data::unsecure non-TCP -- sending data");
...
int M2MConnectionHandlerPimpl::send_to_socket(const unsigned char *buf, size_t len)
{
    ...
    if(_binding_mode == M2MInterface::TCP ||
       _binding_mode == M2MInterface::TCP_QUEUE){
        ...
        tr_debug("M2MConnectionHandlerPimpl::send_to_socket::TCP");
    }else{
        error = _mbed_socket->send_to(buf, len,_resolved_Address,_server_port);
        tr_debug("M2MConnectionHandlerPimpl::send_to_socket::non-TCP");
    }

    if( SOCKET_ERROR_WOULD_BLOCK == error ){
        return M2MConnectionHandler::CONNECTION_ERROR_WANTS_WRITE;
    }else if( SOCKET_ERROR_NONE != error ){
        tr_debug("M2MConnectionHandlerPimpl::send_to_socket::SOCKET_ERROR");
        return -1;
    }else{
        tr_debug("M2MConnectionHandlerPimpl::send_to_socket bytes: %d", len);
        return len;
    }
}
...
int M2MConnectionHandlerPimpl::receive_from_socket(unsigned char *buf, size_t len)
{
    ...
    if(_binding_mode == M2MInterface::TCP ||
       _binding_mode == M2MInterface::TCP_QUEUE){
        ...
        tr_debug("M2MConnectionHandlerPimpl::TCP");
    }else{
    ...
    if( SOCKET_ERROR_WOULD_BLOCK == error ){
        ...
        tr_debug("M2MConnectionHandlerPimpl::SOCKET_ERROR");
        return -1;
    }else{
        tr_debug("M2MConnectionHandlerPimpl::receive_from_socket: %d", len);
        return len;
    }
}
...
void M2MConnectionHandlerPimpl::receive_handshake_handler(Socket */*socket*/)
{
    ...
    if( _is_handshaking ){
        tr_debug("M2MConnectionHandlerPimpl::receive_handshake_handler::handshaking");
        ...
        if( ret == M2MConnectionHandler::CONNECTION_ERROR_WANTS_READ ){ //We wait for next readable event
            ...
            tr_debug("M2MConnectionHandlerPimpl::receive_handshake_handler::handshaking completed");
            _is_handshaking = false;
            ...
            _observer.address_ready(*_socket_address,
                                    _server_type,
                                    _server_port);
            tr_debug("M2MConnectionHandlerPimpl::receive_handshake_handler::address_ready");
            ...
            tr_debug("M2MConnectionHandlerPimpl::observer.socket_error(4)");
        }
    }
}
...
void M2MConnectionHandlerPimpl::receive_handler(Socket */*socket*/)
{
    ...
    if( _use_secure_connection ){
        ...
        tr_debug("M2MConnectionHandlerPimpl::receive_handler::_use_secure_connection");
        ...
        if(rcv_size >= 0){
            ...
            tr_debug("M2MConnectionHandlerPimpl::receive_handler::observer.socket_error(1)");
            return;
        }
    }else{
        tr_debug("M2MConnectionHandlerPimpl::receive_handler::_use_nonsecure_connection");
        socket_error_t error = SOCKET_ERROR_NONE;
        if(_binding_mode == M2MInterface::TCP ||
           _binding_mode == M2MInterface::TCP_QUEUE){
        ...
        if (SOCKET_ERROR_NONE == error) {
            tr_debug("M2MConnectionHandlerPimpl::receive_handler::handle_data");
                ...
                if( receive_length > 4 ){
                    ...
                    if(len > 0) {
                        ...
                        if(buf) {
                            ...
                            tr_debug("M2MConnectionHandlerPimpl::receive_handler::data available");
                            free(buf);
                        }
                    }
                }else{
...
void M2MConnectionHandlerPimpl::dns_handler(Socket */*socket*/, struct socket_addr sa, const char */*domain*/)
{
    ...
    if( _security ){
        tr_debug("M2MConnectionHandlerPimpl::dns_handler::secure");
...
void M2MConnectionHandlerPimpl::error_handler(Socket */*socket*/,
                                              socket_error_t error)
{
    tr_debug("M2MConnectionHandlerPimpl::error_handler");
